...
x <- banner(food, "pescetarian", bys = paste0("rating_", c("meat", "pizza", "sushi", "veg")))
...
Number rows are saved as number values, and character values are saved as text. This allows us to format the numbers to our liking. `write_banners()` by default formats the percentage rows as integers, and the "deff + mose" rows with two decimal places. If you'd prefer them presented as raw, full-decimal-place values, use `format_numbers = FALSE`.
