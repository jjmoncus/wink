...
crosstab2 <- function(data,
  ...
  # unfortunately, have to duplicate wt logic for the sake of `by_params`
  # can maybe remove it from `get_totals` later, but for now, keep
  if (!is.null(weight)) {
  ...
  # unfortunately, have to do the `na` handling in here, too for the sake of `by_params`
  if (na.rm) {
    data <- data %>% filter(!is.na(!!sym_var))
    data <- data %>% filter(!is.na(!!sym_by))
  } else {
  ...
  # gather levels AFTER potentially casting NA
  var_levels <- levels(data[[var]])
  by_levels <- levels(data[[by]])
  # ------------------------------------------- #
  # ------ calculating table params ----------- #
  ...
  # this already excludes people who are NA, if they've been cut
  by_params <- by_levels %>%
    set_names() %>%
    map(function(x) {
      calculate_deff(data %>%
  ...
  # this also already exclues any `by` people who are NA
  n_unweighteds <- by_params %>% filter(by_level %in% by_levels_to_use) %>% pull(n) %>% set_names(by_levels_to_use)
  # using unrounded deffs, then round at the end if needed
  deffs <- by_params %>% filter(by_level %in% by_levels_to_use) %>% pull(deff) %>% set_names(by_levels_to_use)
  # moses <- by_params %>% filter(by_level %in% by_levels_to_use) %>% pull(moe) %>% set_names(by_levels_to_use)
  ...
  if (length(by_levels_to_use) == 0) {
    ...
    baby_crosstab <- baby_crosstab %>%
      select(-any_of(by_levels_to_cut)) %>%
      # this is where we finally apply exclude_var
      filter(!str_detect(!!sym_var, exclude_var)) %>%
      structure("var" = var,
                "by_levels_used" = by_levels_to_use,
...
#' @importFrom purrr pmap_lgl
#'
sig_test_table2 <- function(baby_crosstab,
                           n_unweighteds,
                           deffs,
                           col_start = 3) {
  ...
  # initialize empty dataframe of spaces for sig results
  letters_df <- matrix("",
                       nrow = length(var_levels),
                       ncol = length(by_levels_used))
  ...
  nums <- baby_crosstab %>% select(-any_of(var))

  for (i in 1:length(by_levels_used)) {
    p1s <- nums[, i] / 100
    n1 <- n_unweighteds[i]
    deff1 <- deffs[i]
    letters_i <- letters_df[,i]

    for (j in i:length(by_levels_used)) {
      if (identical(j, i))
        next
      ...
      n2 <- n_unweighteds[j]
      deff2 <- deffs[j]
      letters_j <- letters_df[,j]

      sig <- hardcode_sig_test(p1s, n1, deff1, p2s, n2, deff2)

      # only declare a letter for the cell where percentage is bigger
      # (so we convey the same sig info twice)
      letter_for_i <- sig & (p1s > p2s)
      letter_for_j <- sig & (p1s < p2s)
      # diff_checks[is.na(diff_checks)] <- FALSE

      letters_i[letter_for_i] <- paste(letters_i[letter_for_i], col_letters[j]) %>%
        str_trim()
      letters_j[letter_for_j] <- paste(letters_j[letter_for_j], col_letters[i]) %>%
        str_trim()

      letters_df[, j] <- letters_j
    }
    letters_df[, i] <- letters_i
  }

  dimnames(letters_df) <- list(var_levels,
                               by_levels_used)

  return(as.data.frame(letters_df) %>% rownames_to_column(var = var))
}
